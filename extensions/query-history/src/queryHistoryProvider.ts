...
export class QueryHistoryProvider implements vscode.TreeDataProvider<QueryHistoryNode> {
	...
	constructor() {
		azdata.queryeditor.registerQueryEventListener({
			onQueryEvent: (type: azdata.queryeditor.QueryEventType, document: azdata.queryeditor.QueryDocument, args: azdata.ResultSetSummary | string | undefined, queryInfo?: azdata.queryeditor.IQueryInfo) => {
				if (queryInfo && type === 'queryStop') {
					const queryText = queryInfo.text ?? '';
					this._queryHistoryNodes.push(new QueryHistoryNode(queryText, '', queryText, document.uri, new Date(), 'sqltools2019-3', queryInfo.messages.find(m => m.isError) ? false : true));
