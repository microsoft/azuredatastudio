...
export async function launchAddSqlBindingQuickpick(uri: vscode.Uri | undefined): Promise<void> {
	...
	const selectedBinding = await promptForBindingType();
	...
	const objectName = await promptForObjectName(selectedBinding.type);
	...
	let connectionStringSettingName = await promptForConnectionStringSetting(projectUri);
