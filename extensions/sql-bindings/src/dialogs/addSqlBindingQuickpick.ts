...
export class SqlBindingConfig {
	public constructor(private readonly vscodeMssqlApi: IExtension) {
	}

	public async launchAddSqlBindingQuickpick(uri: vscode.Uri | undefined): Promise<void> {
		...
		const selectedBindingType = await this.promptForBindingType();
		...
		const objectName = await this.promptForObjectName(selectedBindingType);
		...
		let connectionStringSettingName = await this.promptForConnectionStringSetting(projectUri);
		...
		// 5. insert binding
		try {
			const result = await azureFunctionsService.addSqlBinding(selectedBindingType, uri.fsPath, azureFunctionName, objectName, connectionStringSettingName);
			...
			TelemetryReporter.createActionEvent(TelemetryViews.SqlBindingsQuickPick, TelemetryActions.finishAddSqlBinding)
				.withAdditionalProperties({ bindingType: BindingType[selectedBindingType] })
				.send();
		} catch (e) {
			void vscode.window.showErrorMessage(utils.getErrorMessage(e));
			TelemetryReporter.sendErrorEvent(TelemetryViews.SqlBindingsQuickPick, TelemetryActions.finishAddSqlBinding);
	...
	public async promptForObjectName(bindingType: BindingType): Promise<string | undefined> {
	...
	public async promptForConnectionStringSetting(projectUri: vscode.Uri | undefined): Promise<string | undefined> {
				...
				if (selectedSetting.label === constants.createNewLocalAppSettingWithIcon) {
					...
					while (true) {
						...
						if (selectedConnectionStringMethod === constants.userConnectionString) {
							...
							connectionInfo = await this.vscodeMssqlApi.promptForConnection(true);
							if (!connectionInfo) {
							...
							try {
								...
								if (connectionInfo.authenticationType === 'SqlLogin' && connectionInfo.password) {
									...
									if (includePassword === constants.yesString) {
										...
										connectionString = await this.vscodeMssqlApi.getConnectionString(connectionDetails, true, false);
									}
								}
								...
								if (includePassword !== constants.yesString) {
									connectionString = await this.vscodeMssqlApi.getConnectionString(connectionDetails, false, false);
								}
							} catch (e) {
