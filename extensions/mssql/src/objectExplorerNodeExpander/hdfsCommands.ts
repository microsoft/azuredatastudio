        ...
        try {
            ...
            if (folderNode) {
                let options: vscode.OpenDialogOptions = {
                    ...
                    canSelectMany: true,
                    openLabel: LocalizedConstants.lblUploadFiles,
                    filters: filter
                };
...
export class DeleteFilesCommand extends Command {
    ...
    private async deleteFolder(node: FolderNode): Promise<void> {
        if (node) {
            let confirmed = await this.confirmDelete(LocalizedConstants.msgDeleteFolder);
            if (confirmed) {
    ...
    private async deleteFile(node: FileNode): Promise<void> {
        if (node) {
            let confirmed = await this.confirmDelete(LocalizedConstants.msgDeleteFile);
            if (confirmed) {
                await node.delete();
            }
        }
...
export class ConnectTask {
    ...
    private createFromProfile(profile: sqlops.IConnectionProfile): Promise<void> {
        ...
        if (profile.providerName === constants.hadoopKnoxProviderName && connection.host) {
        ...
        let questions: IQuestion[] = [
            {
                ...
                message: LocalizedConstants.msgSetWebHdfsHost,
                default: 'localhost:50070'
            },
            {
                type: QuestionTypes.input,
                name: constants.hdfsUser,
                message: LocalizedConstants.msgSetWebHdfsUser,
                default: 'root'
            }];
