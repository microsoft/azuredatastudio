...
async function handleObjectPropertiesDialogCommand(context: azdata.ObjectExplorerContext, service: IObjectManagementService): Promise<void> {
	...
	try {
		...
		const objectType = context.nodeInfo ? context.nodeInfo.nodeType === 'Database' ? ObjectManagement.NodeType.DatabaseProperties : context.nodeInfo.nodeType as ObjectManagement.NodeType : ObjectManagement.NodeType.Server;
