...
export class Keychain {
	...
	async setPassword(credentialId: string, password: string): Promise<void> {
		try {
			return await this.keytar.setPassword(SERVICE_ID, credentialId, password);
		} catch (e) {
	...
	async getPassword(credentialId: string): Promise<string | null | undefined> {
		try {
			return await this.keytar.getPassword(SERVICE_ID, credentialId);
		} catch (e) {
			// Ignore
	...
	async deletePassword(credentialId: string): Promise<boolean> {
		try {
			return await this.keytar.deletePassword(SERVICE_ID, credentialId);
		} catch (e) {
			// Ignore
