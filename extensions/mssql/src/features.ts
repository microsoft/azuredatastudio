...
export class DiagnosticsServiceFeature extends SqlOpsFeature<undefined> {

	private static readonly messageTypes: RPCMessageType[] = [
		contracts.GetDiagnosticsRequest.type,
	];

	constructor(client: SqlOpsDataClient) {
		super(client, DiagnosticsServiceFeature.messageTypes);
	}
	...
	protected registerProvider(options: undefined): Disposable {
		...
		const getDiagnosticsResults = (diagnosticItem: azdata.window.ITroubleshooterItem): Thenable<azdata.diagnostics.GetDiagnosticsResult> => {
			const params: contracts.GetDiagnosticsParams = {};
			return client.sendRequest(contracts.GetDiagnosticsRequest.type, params).then(
				r => r,
				e => {
					client.logFailedRequest(contracts.GetDiagnosticsRequest.type, e);
					return Promise.reject(e);
				}
			);
		};
		return azdata.diagnostics.registerDiagnosticsProvider({
			targetProviderId: client.providerId,
