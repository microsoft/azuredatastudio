	...
	public async start(context: AppContext): Promise<SqlOpsDataClient> {
		try {
			...
			this.client.onReady().then(() => {
				...
				statusView.text = localize('serviceStartedStatusMsg', "Service Started");
				setTimeout(() => {
					statusView.hide();
				}, 1500);
			...
			statusView.show();
			statusView.text = localize('startingServiceStatusMsg', "Starting service");
			this.client.start();
			await this.activateFeatures(context);
			return this.client;
		} catch (e) {
			Telemetry.sendTelemetryEvent('ServiceInitializingFailed');
			vscode.window.showErrorMessage(localize('failedToStartServiceErrorMsg', "Failed to start Sql tools service"));
			throw e;
		}
	}
