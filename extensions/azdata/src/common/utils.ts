 ...
 */
export function searchForCmd(exe: string): Promise<string | undefined> {
	// Note : This is separated out to allow for easy test stubbing
	return new Promise<string | undefined>((resolve, reject) => which(exe, (err, path) => err ? reject(err) : resolve(path)));
}
