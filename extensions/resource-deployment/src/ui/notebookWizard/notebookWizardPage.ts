...
import { getTextComponent, initializeWizardPage, InputComponent, InputComponents, setModelValues, Validator } from '../modelViewUtils';
...
export class NotebookWizardPage extends WizardPageBase<NotebookWizard> {
	...
	constructor(
		...
		if (
			this.pageInfo.isSummaryPage &&
			!this.wizard.wizardInfo.isSummaryPageAutoGenerated
		) {
	...
	public onEnter() {
		if (this.pageInfo.isSummaryPage) {
			this.inputComponentOriginalValues.forEach(
				(originalValue: string, name: string) => {
					const textComponent = getTextComponent(name, this.inputComponents);
					if (textComponent && originalValue.match(/\$\(.*\)/g)) {
						textComponent.value = this.wizard.model.substituteVariableValues(
							originalValue
						);
					}
				}
