	...
	async validate(): Promise<boolean> {
		try {
			...
			if (this._targetTypeRadioCardGroup?.selectedCardId === constants.Project) {
				if (!await this.validateFile(this._filePathTextBox!.value!, constants.Project.toLowerCase())) {
					return false;
				}
				...
				if (!await this.validateFile(this._filePathTextBox!.value!, constants.Workspace.toLowerCase())) {
					return false;
				}
			}
