...
export class DatabaseSelectorPage extends MigrationWizardPage {
	...
	public async createRootContainer(view: azdata.ModelView): Promise<azdata.FlexContainer> {
		...
		if (!this.migrationStateModel._preselectAllDatabasesForAssessment) {
			...
			// TO-DO: even though all rows are checked, the "check all" box in the table header isn't checked
			// (but it works once you uncheck and recheck something though - probably a UI bug)
		}
