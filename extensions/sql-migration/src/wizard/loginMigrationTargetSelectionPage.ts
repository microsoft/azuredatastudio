...
import { AuthTypeLookup, collectSourceLogins, collectTargetLogins, getSourceConnectionId, getSourceConnectionProfile, isSourceConnectionSysAdmin, LoginTableInfo } from '../api/sqlUtils';
...
import { CategoryValue } from 'dataprotocol-client/lib/types';
...
export class LoginMigrationTargetSelectionPage extends MigrationWizardPage {
	...
	private _createResourceAuthenticationContainer(): azdata.FlexContainer {
		...
		this._authenticationTypeDropdown.values = authTypes;
		this._disposables.push(
			this._authenticationTypeDropdown.onValueChanged(async (value) => {
				if (value && value !== 'undefined') {
					this.migrationStateModel._targetAuthType = AuthTypeLookup[authTypes[value!.index].name];
				}
			}));
