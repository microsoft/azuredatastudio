...
export class IntergrationRuntimePage extends MigrationWizardPage {
	...
	public async onPageEnter(): Promise<void> {
		...
		this.wizard.registerNavigationValidator((pageChangeInfo) => {
			...
			const state = this.migrationStateModel.migrationController.properties.integrationRuntimeState;
	...
	private migrationControllerDropdownsContainer(): azdata.FlexContainer {
		...
		this.migrationControllerDropdown.onValueChanged(async (value) => {
			if (value.selected) {
				...
				this.migrationStateModel.migrationController = this.migrationStateModel.getMigrationController(value.index);
		...
		try {
			...
			if (this.migrationStateModel.migrationController) {
				this.migrationControllerDropdown.value = {
					name: this.migrationStateModel.migrationController.id,
	...
	private async loadControllerStatus(): Promise<void> {
		...
		if (this.migrationStateModel.migrationController) {
			const controller = await getMigrationController(
				this.migrationStateModel.azureAccount,
				this.migrationStateModel._targetSubscription,
				this.migrationStateModel.migrationController.properties.resourceGroup,
				this.migrationStateModel.migrationController.properties.location,
				this.migrationStateModel.migrationController.name);
			this.migrationStateModel.migrationController = controller;
			...
			refreshKey1Button.onDidClick((e) => {
			});
			...
			refreshKey2Button.onDidClick((e) => {
			});
