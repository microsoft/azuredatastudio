...
export class UriHandlerService implements vscode.UriHandler {
	...
	async handleUri(uri: vscode.Uri): Promise<void> {
		...
		if (uri.path === '/test') {
			...
			const selectedDatabases = params.find(param => param[0] === 'databases')?.[1];

			console.log("test!");
			void vscode.window.showInformationMessage("test!");
			void vscode.window.showInformationMessage(params.join(','));
			vscode.commands.executeCommand('sqlmigration.start', selectedDatabases);
