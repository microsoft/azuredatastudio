...
export class LanguageClientErrorHandler implements ErrorHandler {
	...
	showOnErrorPrompt(): void {
		Telemetry.sendTelemetryEvent(ServerUtils.ServerServiceName + 'Crash');
		void vscode.window.showErrorMessage(
			localize('serviceCrashMessage', "{0} component exited unexpectedly. Please restart Azure Data Studio.", ServerUtils.ServerServiceName),
			viewKnownIssuesAction).then(action => {
				if (action && action === viewKnownIssuesAction) {
					void vscode.env.openExternal(vscode.Uri.parse(ServerUtils.ServiceCrashLink));
				}
			});
	}
