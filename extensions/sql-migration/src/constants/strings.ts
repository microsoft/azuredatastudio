...
export const LOGIN_MIGRATIONS_TARGET_SELECTION_PAGE_DATA_MIGRATION_WARNING = localize('sql.login.migration.wizard.target.data.migration.warning', "We recommend migrating all your databases(s) to the target before starting the login migration to avoid failures in the process. Also, if the source and database names are not the same, then it is possible that some permissions may not be applied properly.");
