...
describe('Notebook Extension Python Installation', function () {
	...
	before(async function () {
		...
		await jupyterController.jupyterInstallation.startInstallProcess(false, { installPath: pythonInstallDir, existingPython: false, specificPackages: [] });
	...
	it('Use Existing Python Installation', async function () {
		...
		await install.startInstallProcess(false, { installPath: existingPythonPath, existingPython: true, specificPackages: [] });
		should(JupyterServerInstallation.isPythonInstalled()).be.true();
		...
		await install.startInstallProcess(false, { installPath: pythonInstallDir, existingPython: false, specificPackages: [] });
		should(JupyterServerInstallation.isPythonInstalled()).be.true();
