...
export class PerFolderServerInstance implements IServerInstance {
	...
	public async stop(): Promise<void> {
		try {
			...
			if (this._isStarted) {
				...
				let stopCommand = `"${install.pythonExecutable}" -s c:\\spython\\nb.py stop ${this._port}`;
	...
	protected async startInternal(): Promise<void> {
		...
		let startCommand = `"${this.options.install.pythonExecutable}" -s c:\\spython\\nb.py --no-browser --no-mathjax --notebook-dir "${notebookDirectory}" --port=${port} --NotebookApp.token=${token}`;
