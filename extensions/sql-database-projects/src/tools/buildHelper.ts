	...
	public constructBuildArguments(projectPath: string, buildDirPath: string, sqlProjStyle: ProjectType): string {
		...
		if (utils.getAzdataApi()) {
			if (sqlProjStyle === mssql.ProjectType.SdkStyle) {
				return `/p:NetCoreBuild=true /p:SystemDacpacsLocation=${buildDirPath} -${constants.runWithVerbose}`;
			} else {
				return `/p:NetCoreBuild=true /p:NETCoreTargetsPath=${buildDirPath} /p:SystemDacpacsLocation=${buildDirPath} -${constants.runWithVerbose}`;
			}
		} else {
