...
describe('ProjectsController', function (): void {
	...
	describe('project controller operations', function (): void {
		describe('Project file operations and prompting', function (): void {
			...
			it('Should show error if trying to add a file that already exists', async function (): Promise<void> {
				...
				const projController = new ProjectsController(testContext.outputChannel);
				let project = await testUtils.createTestProject(this.currentTest, baselines.newProjectFileBaseline);
			...
			it('Should be able to add folder with reserved name as long as not at project root', async function (): Promise<void> {
				...
				const projController = new ProjectsController(testContext.outputChannel);
				let project = await testUtils.createTestProject(this.currentTest, baselines.openProjectFileBaseline);
				const projectRoot = new ProjectRootTreeItem(project);
			...
			it.skip('Should delete nested ProjectEntry from node', async function (): Promise<void> {
				let proj = await testUtils.createTestProject(this.currentTest, templates.newSqlProjectTemplate);
			...
			it('Should delete database references', async function (): Promise<void> {
				...
				let proj = await testUtils.createTestProject(this.currentTest, baselines.openProjectFileBaseline);
				const projController = new ProjectsController(testContext.outputChannel);
			...
			it('Should reload correctly after changing sqlproj file', async function (): Promise<void> {
				...
				const sqlProjPath = await testUtils.createTestSqlProjFile(this.currentTest, baselines.newProjectFileBaseline, folderPath);
			...
			it('Should change target platform', async function (): Promise<void> {
				...
				const projController = new ProjectsController(testContext.outputChannel);
				const sqlProjPath = await testUtils.createTestSqlProjFile(this.currentTest, baselines.openProjectFileBaseline);
		...
		describe('Publishing and script generation', function (): void {
			...
			it('Callbacks are hooked up and called from Publish dialog', async function (): Promise<void> {
				const projectFile = await testUtils.createTestSqlProjFile(this.currentTest, baselines.openProjectFileBaseline)
				const projFolder = path.dirname(projectFile);
				await testUtils.createTestDataSources(this.currentTest, baselines.openDataSourcesBaseline, projFolder);
			...
			it('Should copy dacpac to temp folder before publishing', async function (): Promise<void> {
				...
				projController.setup(x => x.buildProject(TypeMoq.It.isAny())).returns(async () => {
					builtDacpacPath = await testUtils.createTestFile(this.currentTest, fakeDacpacContents, 'output.dacpac');
					return builtDacpacPath;
				});
				sinon.stub(utils, 'getDacFxService').resolves(testContext.dacFxService.object);

				const proj = await testUtils.createTestProject(this.currentTest, baselines.openProjectFileBaseline);
	...
	describe('Create project from database operations and dialog', function (): void {
		...
		it('Should create list of all files and folders correctly', async function (): Promise<void> {
			const testFolderPath = await testUtils.createDummyFileStructure(this.currentTest);
		...
		it('Should error out for inaccessible path', async function (): Promise<void> {
			...
			let testFolderPath = await testUtils.generateTestFolderPath(this.currentTest);
		...
		it('Should set model filePath correctly for ExtractType = File', async function (): Promise<void> {
			let folderPath = await testUtils.generateTestFolderPath(this.currentTest);
	...
	describe('Add database reference', function (): void {
		...
		it('Callbacks are hooked up and called from Add database reference dialog', async function (): Promise<void> {
			const projPath = path.dirname(await testUtils.createTestSqlProjFile(this.currentTest, baselines.openProjectFileBaseline));
		...
		it.skip('Should not allow adding circular project references', async function (): Promise<void> {
			const projPath1 = await testUtils.createTestSqlProjFile(this.currentTest, baselines.openProjectFileBaseline);
			const projPath2 = await testUtils.createTestSqlProjFile(this.currentTest, baselines.newProjectFileBaseline);
			const projController = new ProjectsController(testContext.outputChannel);
	...
	describe('AutoRest generation', function (): void {
		...
		it.skip('Should create project from autorest-generated files', async function (): Promise<void> {
			const parentFolder = await testUtils.generateTestFolderPath(this.currentTest);
			await testUtils.createDummyFileStructure(this.currentTest);
			...
			projController.setup(x => x.generateAutorestFiles(TypeMoq.It.isAny(), TypeMoq.It.isAny())).returns(async () => {
				await testUtils.createDummyFileStructure(this.currentTest, true, fileList, newProjFolder);
				await testUtils.createTestFile(this.currentTest, 'SELECT \'This is a post-deployment script\'', constants.autorestPostDeploymentScriptName, newProjFolder);
	...
	describe('Move file', function (): void {
		it('Should move a file to another folder', async function (): Promise<void> {
			...
			let proj = await testUtils.createTestProject(this.currentTest, baselines.openSdkStyleSqlProjectBaseline);
		...
		it('Should not allow moving files between projects', async function (): Promise<void> {
			...
			let proj1 = await testUtils.createTestProject(this.currentTest, baselines.openSdkStyleSqlProjectBaseline);
			let proj2 = await testUtils.createTestProject(this.currentTest, baselines.openSdkStyleSqlProjectBaseline);
	...
	describe('Rename file', function (): void {
		...
		it('Should rename a pre and post deploy script', async function (): Promise<void> {
			let proj = await testUtils.createTestProject(this.currentTest, baselines.newSdkStyleProjectSdkNodeBaseline);
	...
	describe('SqlCmd Variables', function (): void {
		it('Should delete sqlcmd variable', async function (): Promise<void> {
			let project = await testUtils.createTestProject(this.currentTest, baselines.openSdkStyleSqlProjectBaseline);
