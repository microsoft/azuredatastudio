...
export async function getDefaultPublishDeploymentOptions(project: ISqlProject): Promise<mssql.DeploymentOptions | vscodeMssql.DeploymentOptions> {
	...
	if (getAzdataApi()) {
		result.defaultDeploymentOptions.excludeObjectTypes.value = (result.defaultDeploymentOptions as mssql.DeploymentOptions).excludeObjectTypes.value?.filter(x => x !== result.defaultDeploymentOptions.includeObjects[constants.DatabaseScopedCredentials]);
	} else {
		result.defaultDeploymentOptions.excludeObjectTypes.value = (result.defaultDeploymentOptions as vscodeMssql.DeploymentOptions).excludeObjectTypes.value?.filter(x => x !== result.defaultDeploymentOptions.includeObjects[constants.DatabaseScopedCredentials]);
	}
