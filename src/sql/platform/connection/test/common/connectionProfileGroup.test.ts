...
suite('SQL ConnectionProfileGroup tests', () => {
	...
	let Groups11 = 'G11';
	let Groups2 = 'G2';
	let group1Node: ConnectionGroup;
	let group11Node: ConnectionGroup;
	let group2Node: ConnectionGroup;
	setup(() => {
		root = new ConnectionGroup(ConnectionGroup.RootGroupName, undefined, ConnectionGroup.RootGroupName, undefined, undefined);

		group1Node = new ConnectionGroup(Groups1, root, Groups1, undefined, undefined);
		group2Node = new ConnectionGroup(Groups2, root, Groups2, undefined, undefined);
		group11Node = new ConnectionGroup(Groups11, root, Groups11, undefined, undefined);
	...
	test('getGroupFullNameParts should return a list With ROOT in it given an empty string', () => {
		...
		let expected: string[] = [ConnectionGroup.RootGroupName];
		let actual = ConnectionGroup.getGroupFullNameParts(groupFullName);
	...
	test('getGroupFullNameParts should add ROOT as first item if not added already and string starts with /', () => {
		...
		let expected: string[] = [ConnectionGroup.RootGroupName, 'Groups', 'Group1'];
		let actual = ConnectionGroup.getGroupFullNameParts(groupFullName);
		assert.deepEqual(actual, expected);
	});
	...
	test('isRoot should return true given empty string', () => {
		...
		let actual = ConnectionGroup.isRoot(name);
		assert.deepEqual(actual, expected);
	});
	...
	test('sameGroupName should return true given root', () => {
		...
		let name2: string = '';
		let expected: boolean = true;
		let actual = ConnectionGroup.sameGroupName(name1, name2);
		assert.deepEqual(actual, expected);
	});
