...
suite('notebook model', function (): void {
	...
	test('Should not delete custom metadata', async function (): Promise<void> {
		...
		defaultModelOptions.contentManager = mockContentManager.object;
		// When I initialize the model
		let model = new NotebookModel(defaultModelOptions, undefined, logService, undefined, undefined, queryConnectionService.object, configurationService, undefined);
		await model.loadContents();
	...
	async function loadModelAndStartClientSession(notebookContent: nb.INotebookContents): Promise<NotebookModel> {
		...
		let model = new NotebookModel(options, undefined, logService, undefined, new NullAdsTelemetryService(), queryConnectionService.object, configurationService, undefined, capabilitiesService);
