	...
	public async onInsertButtonClick(event: MouseEvent, type: MarkdownButtonType): Promise<void> {
		...
		if (type === MarkdownButtonType.LINK_PREVIEW) {
			...
			if (this.cellModel.currentMode !== CellEditModes.WYSIWYG) {
				...
				document.execCommand('insertHTML', false, `<a href="${encodeURI(linkUrl)}" title="${encodeURI(linkUrl)}" is-encoded="true" is-absolute=${notebookLink.isAbsolutePath}>${escape(linkCalloutResult?.insertUnescapedLinkLabel)}</a>`);
				return;
			}
