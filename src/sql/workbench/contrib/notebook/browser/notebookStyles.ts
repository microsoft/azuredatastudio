...
export function registerNotebookThemes(overrideEditorThemeSetting: boolean, configurationService: IConfigurationService): IDisposable {
	return registerThemingParticipant((theme: IColorTheme, collector: ICssStyleCollector) => {
		...
		const codeCellParameterBackgroundColor = theme.getColor(codeCellParameterBackground);
		const codeCellParameterTextColor = theme.getColor(codeCellParameterText);
		if (codeCellParameterBackgroundColor) {
			collector.addRule(`code-component .parameter span { background-color: ${codeCellParameterBackgroundColor}; color: ${codeCellParameterTextColor}`);
		}
	});
}
