...
export class QueryEditorLanguageAssociation implements ILanguageAssociation {
	...
	async convertInput(activeEditor: IEditorInput): Promise<QueryEditorInput | undefined> {
		...
		if (activeEditor instanceof FileEditorInput) {
			...
			queryEditorInput = await this.queryEditorService.newSqlEditor({ resource: this.editorService.isOpen(activeEditor) ? activeEditor.resource : undefined, open: false, initalContent: content }, profile ? profile.providerName : undefined) as UntitledQueryEditorInput;
		} else {
			return undefined;
		}
