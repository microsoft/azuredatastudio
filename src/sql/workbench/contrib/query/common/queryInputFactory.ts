...
export class QueryEditorLanguageAssociation implements ILanguageAssociation {
	...
	convertInput(activeEditor: IEditorInput): QueryEditorInput | undefined {
		...
		if (activeEditor instanceof FileEditorInput) {
			const input = this.editorService.createInput({ resource: activeEditor.getResource() }) as FileEditorInput;
			queryEditorInput = this.instantiationService.createInstance(FileQueryEditorInput, '', input, queryResultsInput);
		} else if (activeEditor instanceof UntitledTextEditorInput) {
			const input = this.untitledTextEditorService.createOrGet(activeEditor.getResource(), activeEditor.getMode(), activeEditor.getEncoding());
			queryEditorInput = this.instantiationService.createInstance(UntitledQueryEditorInput, '', input, queryResultsInput);
		} else {
			return undefined;
		}
	...
	createBase(activeEditor: UntitledQueryEditorInput | FileQueryEditorInput): IEditorInput {
		if (activeEditor instanceof UntitledQueryEditorInput) {
			return this.untitledTextEditorService.createOrGet(activeEditor.getResource(), activeEditor.getMode(), activeEditor.getEncoding());
		} else {
			return this.editorService.createInput({ resource: activeEditor.getResource() });
		}
	}
}
