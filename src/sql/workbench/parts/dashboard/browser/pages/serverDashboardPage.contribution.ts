...
import { generateDashboardWidgetSchema, generateDashboardTabSchema } from 'sql/workbench/parts/dashboard/browser/pages/dashboardPageContribution';
...
const defaultVal = [
	{
		name: 'Tasks',
		widget: {
			'tasks-widget': [{ name: 'restore', when: '!mssql:iscloud && !mssql:issqlondemand' }, 'configureDashboard', 'newQuery', 'mssqlCluster.task.newNotebook']
		},
		gridItemConfig: {
			sizex: 1,
			sizey: 1
