...
import { SaveResultAction, MaximizeTableAction, RestoreTableAction, ChartDataAction, CopyResultAction, IGridActionContext, SelectAllGridAction } from 'sql/workbench/parts/query/browser/actions';
...
type GridPanelTable<T> = GridTable<T> | HighPerfGridTable<T>;

export class GridPanel {
	private container = document.createElement('div');
	private splitView: ScrollableSplitView;
	private tables: GridPanelTable<any>[] = [];
	...
	private maximizedGrid: GridPanelTable<any>;
	...
	private addResultSet(resultSet: azdata.ResultSetSummary[]) {
		const tables: GridPanelTable<any>[] = [];
		...
		for (const set of resultSet) {
			...
			const highPerf = this.configurationService.getValue<boolean>('sql.results.optimizedTable');
			let table: GridPanelTable<any>;
			if (highPerf) {
