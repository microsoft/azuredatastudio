...
export class ExtHostDataProtocol extends ExtHostDataProtocolShape {
	...
	override $changeConnectionUri(handle: number, newUri: string, oldUri: string): Thenable<void> {
		if (this.uriTransformer) {
		...
		return this._resolveProvider<azdata.QueryProvider>(handle).changeConnectionUri(newUri, oldUri);
	}
