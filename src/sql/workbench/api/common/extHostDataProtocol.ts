...
export class ExtHostDataProtocol extends ExtHostDataProtocolShape {
	...
	override $renameQuery(handle: number, newUri: string, oldUri: string): Thenable<void> {
		if (this.uriTransformer) {
		...
		return this._resolveProvider<azdata.QueryProvider>(handle).renameQuery(newUri, oldUri);
	}
