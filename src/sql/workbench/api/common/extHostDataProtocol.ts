...
export class ExtHostDataProtocol extends ExtHostDataProtocolShape {
	...
	override $changeUriForQuery(handle: number, newUri: string, oldUri: string): Thenable<void> {
		if (this.uriTransformer) {
		...
		return this._resolveProvider<azdata.QueryProvider>(handle).changeUriForQuery(newUri, oldUri);
	}
