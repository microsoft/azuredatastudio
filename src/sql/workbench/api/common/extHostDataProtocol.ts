...
export class ExtHostDataProtocol extends ExtHostDataProtocolShape {
	...
	override $changeConnectionUriForQuery(handle: number, newUri: string, oldUri: string): Thenable<void> {
		if (this.uriTransformer) {
		...
		return this._resolveProvider<azdata.QueryProvider>(handle).changeConnectionUriForQuery(newUri, oldUri);
	}
