...
export class ExtHostErrorDiagnostics extends ExtHostErrorDiagnosticsShape {
	...
	public override $handleConnectionError(handle: number, errorCode: number, errorMessage: string, callStack: string, connection: azdata.connection.ConnectionProfile): Thenable<azdata.diagnostics.ConnectionDiagnosticsResult> {
		let provider = this._providers[handle];
		if (provider === undefined) {
			return Promise.resolve({ handled: false });
		}
		else {
			return provider.provider.handleConnectionError(errorCode, errorMessage, callStack, connection);
		}
	}
