...
import 'vs/css!./media/horizontalSelector';
import { IComponent, IComponentDescriptor, IModelStore, ComponentEventType } from 'sql/platform/dashboard/browser/interfaces';
import { deepClone } from 'vs/base/common/objects';

@Component({
	templateUrl: decodeURI(require.toUrl('./horizontalSelector.component.html'))

})
export default class HorizontalSelectorCard extends ComponentBase implements IComponent, OnDestroy {
	@Input() descriptor: IComponentDescriptor;
	@Input() modelStore: IModelStore;
	@ViewChildren('selectorDiv') cardElements: QueryList<ElementRef>;
	...
	private getCardById(cardId: string): azdata.HorizontalSelectorCard {
	...
	public get cards(): azdata.HorizontalSelectorCard[] {
	...
	private getSpecficProperties(): azdata.HorizontalSelectorComponentProperties {
		return this.getProperties<azdata.HorizontalSelectorComponentProperties>();
	}

	public getIconClass(cardId: string): string {
		if (!this.iconClasses[cardId]) {
			this.iconClasses[cardId] = `horizontal-icon icon ${createIconCssClass(this.getCardById(cardId).icon)}`;
		}
	...
	public selectCard(cardId: string): void {
		...
		this.setPropertyFromUI<azdata.HorizontalSelectorComponentProperties, string | undefined>((props, value) => props.selectedCardId = value, cardId);
	...
	public onLinkClick(cardId: string, textContents: azdata.HorizontalSelectorText): void {
		this.fireEvent({
