...
export async function scriptSelect(connectionProfile: IConnectionProfile, metadata: azdata.ObjectMetadata, connectionService: IConnectionManagementService, queryEditorService: IQueryEditorService, scriptingService: IScriptingService): Promise<boolean> {
	...
	if (result && result.script) {
		let connectionProviderDetails = { connectionProviderName: connectionProfile.providerName, languageMode: connectionService.getProviderLanaguageMode(connectionProfile.providerName) };
		const owner = await queryEditorService.newSqlEditor({ initalContent: result.script }, connectionProviderDetails);
