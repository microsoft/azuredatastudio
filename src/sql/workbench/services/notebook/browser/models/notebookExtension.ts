...
export class NotebookExtension<ViewMetadata, CellMetadata> {
	readonly version = 1;
	readonly extensionName = 'azuredatastudio';
	readonly extensionNamespace = 'extensions';

	public getNotebookMetadata(notebook: INotebookModel): ViewMetadata {
		const metadata = notebook.getMetaValue(this.extensionNamespace) || {};
		return metadata[this.extensionName] as ViewMetadata;
	}

	public setNotebookMetadata(notebook: INotebookModel, metadata: ViewMetadata) {
		const meta = {};
	...
	public getCellMetadata(cell: ICellModel): CellMetadata {
		const namespaceMeta = cell.metadata[this.extensionNamespace] || {};
		return namespaceMeta[this.extensionName] as CellMetadata;
	}

	public setCellMetadata(cell: ICellModel, metadata: CellMetadata) {
		const meta = {};
		meta[this.extensionName] = metadata;
