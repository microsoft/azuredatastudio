...
export class ConnectionController implements IConnectionComponentController {
	...
	constructor(
		...
		this._connectionWidget = this._instantiationService.createInstance(ConnectionWidget, specialOptions, {
			...
			onFetchDatabases: (serverName: string, authenticationType: string, userName?: string, password?: string, token?: string) => this.onFetchDatabases(
				serverName, authenticationType, userName, password, token).then(result => {
					return result;
				}),
	...
	protected async onFetchDatabases(serverName: string, authenticationType: string, userName?: string, password?: string, token?: string): Promise<string[]> {
		...
		tempProfile.azureAccount = token;
