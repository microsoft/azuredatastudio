...
export default class QueryRunner extends Disposable {
	 ...
	 */
	async copyResults(selections: Slick.Range[], batchId: number, resultId: number, includeHeaders?: boolean): Promise<azdata.CopyResultsResults> {
		return await this.queryManagementService.copyResults({
			ownerUri: this.uri,
			batchIndex: batchId,
