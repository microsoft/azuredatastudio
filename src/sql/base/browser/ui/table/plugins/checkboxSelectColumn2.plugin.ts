...
export class CheckboxSelectColumn<T extends Slick.SlickData> implements Slick.Plugin<T> {
	...
	private handleClick(e: DOMEvent, args: Slick.OnClickEventArgs<T>): void {
		...
		const currentValue = this.getCheckboxPropertyValue(args.row);
		this.setCheckboxPropertyValue(args.row, !currentValue.checked);
		this._grid.invalidateRow(args.row);
		this._grid.render();
		this._grid.setActiveCell(args.row, this.index);
		this.checkSelectAll();
		this.updateSelectedRows();
		e.stopPropagation();
		e.stopImmediatePropagation();
		e.preventDefault();
		this._onChange.fire({ checked: false, row: args.row, column: this.index });
