...
import { /*AzureRestResponse,*/ GetSubscriptionsResult, /*ResourceQueryResult, GetBlobContainersResult, GetFileSharesResult, HttpRequestMethod, GetBlobsResult, GetStorageAccountAccessKeyResult,*/ AzureAccount } from 'azurecore';
...
export async function getSubscriptions(account?: AzureAccount, ignoreErrors: boolean = false): Promise<GetSubscriptionsResult> {
	...
	await Promise.all(account.properties.tenants.map(async (tenant: { id: string; }) => {
		try {
			result.subscriptions.push(...await getSubscriptionsImpl(account, [tenant.id]));
		} catch (err) {
...
export async function getSubscriptionsImpl(account: AzureAccount, tenantIds?: string[]): Promise<azureResource.AzureResourceSubscription[]> {
