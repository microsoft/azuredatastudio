...
KeybindingsRegistry.registerCommandAndKeybindingRule({
	...
	handler: (accessor, resourceOrObject: URI | object | { from: string }) => {
		...
		// {{SQL CARBON EDIT}}
		return save(resource, true, void 0, editorService, accessor.get(IFileService), accessor.get(IUntitledEditorService), accessor.get(ITextFileService), accessor.get(IEditorGroupsService), accessor.get(IQueryEditorService));
	}
});
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
	...
	handler: (accessor, resource: URI | object) => {
		...
		if (resources.length === 1) {
			...
			// {{SQL CARBON EDIT}}
			return save(resources[0], false, void 0, editorService, accessor.get(IFileService), accessor.get(IUntitledEditorService), accessor.get(ITextFileService), accessor.get(IEditorGroupsService), accessor.get(IQueryEditorService));
		}
