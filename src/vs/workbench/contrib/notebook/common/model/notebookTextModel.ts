...
export class NotebookTextModel extends Disposable implements INotebookTextModel {
		...
		for (const { edit, cellIndex } of flattenEdits) {
			switch (edit.editType) {
				...
				case CellEditType.Output:
					...
					if (edit.append) {
						// {{SQL CARBON EDIT}}
						this._spliceNotebookCellOutputs(cell, [cell.outputs.length, 0, edit.outputs.map(op => new NotebookCellOutputTextModel(op))], true, computeUndoRedo);
					} else {
	...
	private _changeCellInternalMetadataPartial(cell: NotebookCellTextModel, internalMetadata: NullablePartialNotebookCellInternalMetadata) {
		...
		for (k in internalMetadata) {
			...
			newInternalMetadata[k] = value as never; // {{SQL CARBON EDIT}} Cast to never instead of any to fix type assertion error
		}
	...
	private _spliceNotebookCellOutputs(cell: NotebookCellTextModel, splice: NotebookCellOutputsSplice, append: boolean, computeUndoRedo: boolean): void {
		...
		this._eventEmitter.emit({
			...
			// append, {{SQL CARBON EDIT}} Remove unused append
