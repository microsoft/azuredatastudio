...
import { IEditorInputWithOptionsAndGroup } from 'vs/workbench/common/editor'; // {{SQL CARBON EDIT}} Cast to avoid compiler errors
...
export class BinaryFileEditor extends BaseBinaryResourceEditor {
	...
	private async openInternal(input: EditorInput, options: IEditorOptions | undefined): Promise<void> {
		if (input instanceof FileEditorInput && this.group && this.group.activeEditor) {
			...
			overridenInput = overridenInput as IEditorInputWithOptionsAndGroup | undefined; // {{SQL CARBON EDIT}} Cast to avoid compiler errors
