...
export class AuthenticationService extends Disposable implements IAuthenticationService {
	...
	async removeAccountSessions(id: string, accountName: string, sessions: AuthenticationSession[], updateAccountList: () => Promise<void> | undefined = undefined): Promise<void> { // {{SQL CARBON EDIT}} - Added updateAccountList delegate to update accounts pane after session sign out
		const authProvider = this._authenticationProviders.get(id);
		if (authProvider) {
			return authProvider.removeAccountSessions(accountName, sessions, updateAccountList); // {{SQL CARBON EDIT}} - Added updateAccountList delegate to update accounts pane after session sign out
		} else {
			throw new Error(`No authentication provider '${id}' is currently registered.`);
		}
	}
