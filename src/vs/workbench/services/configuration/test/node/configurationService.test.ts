	...
	test('initialize a folder workspace from an empty workspace with configuration changes', () => {
		...
		return testObject.reloadConfiguration()
			.then(() => {
				...
				// {{SQL CARBON EDIT}}
				fs.writeFileSync(path.join(parentResource, '1', '.sqlops', 'settings.json'), '{ "initialization.testSetting1": "workspaceValue" }');
	...
	test('initialize a multi root workspace from an empty workspace with configuration changes', () => {
		...
		return testObject.reloadConfiguration()
			.then(() => {
				...
				// {{SQL CARBON EDIT}}
				fs.writeFileSync(path.join(parentResource, '1', '.sqlops', 'settings.json'), '{ "initialization.testSetting1": "workspaceValue1" }');
				fs.writeFileSync(path.join(parentResource, '2', '.sqlops', 'settings.json'), '{ "initialization.testSetting2": "workspaceValue2" }');
	...
	test('initialize a folder workspace from a folder workspace with configuration changes', () => {
		...
		return testObject.initialize(path.join(parentResource, '1'))
			.then(() => {
				...
				// {{SQL CARBON EDIT}}
				fs.writeFileSync(path.join(parentResource, '2', '.sqlops', 'settings.json'), '{ "initialization.testSetting1": "workspaceValue2" }');
				return testObject.initialize(path.join(parentResource, '2'))
					.then(() => {
	...
	test('initialize a multi folder workspace from a folder workspacce triggers change events in the right order', () => {
		...
		return testObject.initialize(folderDir)
			.then(() => {
				...
				// {{SQL CARBON EDIT}}
				fs.writeFileSync(path.join(parentResource, '1', '.sqlops', 'settings.json'), '{ "initialization.testSetting1": "workspaceValue2" }');
				return testObject.initialize({ id: workspaceConfigPath, configPath: workspaceConfigPath })
					.then(() => {
						assert.equal(target.callCount, 4);
						assert.deepEqual((<IConfigurationChangeEvent>target.args[0][0]).affectedKeys, ['initialization.testSetting1']);
...
suite('WorkspaceConfigurationService - Folder', () => {
	...
	test('workspace settings override user settings after defaults are registered ', () => {
		...
		// {{SQL CARBON EDIT}}
		fs.writeFileSync(path.join(workspaceDir, '.sqlops', 'settings.json'), '{ "configurationService.folder.newSetting": "workspaceValue" }');
		return testObject.reloadConfiguration()
			.then(() => {
	...
	test('executable settings are not read from workspace', () => {
		...
		// {{SQL CARBON EDIT}}
		fs.writeFileSync(path.join(workspaceDir, '.sqlops', 'settings.json'), '{ "configurationService.folder.executableSetting": "workspaceValue" }');
		return testObject.reloadConfiguration()
	...
	test('get unsupported workspace settings after defaults are registered', () => {
		// {{SQL CARBON EDIT}}
		fs.writeFileSync(path.join(workspaceDir, '.sqlops', 'settings.json'), '{ "configurationService.folder.anotherExecutableSetting": "workspaceValue" }');
		return testObject.reloadConfiguration()
			.then(() => {
				configurationRegistry.registerConfiguration({
					'id': '_test',
...
suite('WorkspaceConfigurationService - Multiroot', () => {
	...
	test('executable settings are not read from workspace folder', () => {
		...
		// {{SQL CARBON EDIT}}
		fs.writeFileSync(workspaceContextService.getWorkspace().folders[0].toResource('.sqlops/settings.json').fsPath, '{ "configurationService.workspace.testExecutableResourceSetting": "workspaceFolderValue" }');
		return testObject.reloadConfiguration()
	...
	test('executable settings are not read from workspace folder after defaults are registered', () => {
		...
		// {{SQL CARBON EDIT}}
		fs.writeFileSync(workspaceContextService.getWorkspace().folders[0].toResource('.sqlops/settings.json').fsPath, '{ "configurationService.workspace.testNewExecutableResourceSetting": "workspaceFolderValue" }');
		return testObject.reloadConfiguration()
			.then(() => {
				configurationRegistry.registerConfiguration({
					'id': '_test',
	...
	test('get unsupported workspace settings after defaults are registered', () => {
		fs.writeFileSync(workspaceContextService.getWorkspace().folders[0].toResource('.sqlops/settings.json').fsPath, '{ "configurationService.workspace.testNewExecutableResourceSetting2": "workspaceFolderValue" }');
		return jsonEditingServce.write(workspaceContextService.getWorkspace().configuration, { key: 'settings', value: { 'configurationService.workspace.testExecutableSetting': 'workspaceValue' } }, true)
			.then(() => testObject.reloadConfiguration())
			.then(() => {
				configurationRegistry.registerConfiguration({
	...
	test('resource setting in folder is read after it is registered later', () => {
		fs.writeFileSync(workspaceContextService.getWorkspace().folders[0].toResource('.sqlops/settings.json').fsPath, '{ "configurationService.workspace.testNewResourceSetting2": "workspaceFolderValue" }');
			...
			.then(() => {
				...
				return jsonEditingServce.write(workspaceContextService.getWorkspace().configuration, { key: 'settings', value: { 'configurationService.workspace.testResourceSetting': 'workspaceValue' } }, true)
					...
					.then(() => {
						...
						// {{SQL CARBON EDIT}}
						fs.writeFileSync(workspaceContextService.getWorkspace().folders[0].toResource('.sqlops/settings.json').fsPath, '{ "configurationService.workspace.testResourceSetting": "workspaceFolderValue" }');
