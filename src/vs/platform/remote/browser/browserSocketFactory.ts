...
export class BrowserSocketFactory implements ISocketFactory {
	...
	connect(host: string, port: number, query: string, callback: IConnectCallback): void {
		const socket = this._webSocketFactory.create(`${location.protocol === 'https:' ? 'wss' : 'ws'}://${host}:${location.protocol === 'https:' ? '443' : port}/?${query}&skipWebSocketFrames=false`); // {{SQL CARBON EDIT}} # dynamic websocket protocol
