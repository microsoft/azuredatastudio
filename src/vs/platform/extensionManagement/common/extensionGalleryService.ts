			...
			return this.requestService.request({
				...
				return asJson<IRawGalleryQueryResult>(context).then(result => {
					if (result) {
						...
						// const resultCount = r.resultMetadata && r.resultMetadata.filter(m => m.metadataType === 'ResultCount')[0]; {{SQL CARBON EDIT}} comment out
						// const total = resultCount && resultCount.metadataItems.filter(i => i.name === 'TotalCount')[0].count || 0; {{SQL CARBON EDIT}} comment out
